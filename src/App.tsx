...
export default function App() {
  ...
  return <div className="app" dir="rtl"><header className="site-header"><Link className="brand" to="/">سوقي</Link><nav aria-label="التنقل الرئيسي"><Link to="/">الرئيسية</Link><Link to="/products">المنتجات</Link><Link to="/cart">السلة</Link>{user ? <Link to="/account">حسابي</Link> : <><Link to="/login">دخول</Link><Link to="/register">تسجيل</Link></>}</nav></header><Routes><Route path="/" element={<HomePage />} /><Route path="/products" element={<ProductsPage />} /><Route path="/products/:id" element={<ProductDetailsPage />} /><Route path="/cart" element={<CartPage />} /><Route path="/checkout" element={<CheckoutPage />} /><Route path="/order-success" element={<SuccessPage />} /><Route path="/login" element={<LoginPage />} /><Route path="/register" element={<RegisterPage />} /><Route element={<ProtectedRoute />}><Route path="/account" element={<AccountLayout />}><Route index element={<AccountHomePage />} /><Route path="orders" element={<PlaceholderPage title="طلباتي" />} /><Route path="profile" element={<PlaceholderPage title="ملفي الشخصي" />} /></Route><Route element={<RoleRoute roles={['merchant']} />}><Route path="/merchant" element={<PlaceholderPage title="لوحة التاجر" />} /></Route><Route element={<RoleRoute roles={['admin']} />}><Route path="/admin" element={<PlaceholderPage title="لوحة المدير" />} /></Route></Route><Route path="*" element={<NotFoundPage />} /></Routes></div>;
}
